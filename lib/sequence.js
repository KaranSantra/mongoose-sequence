    ...
    return function (next, done) {
      ...
      sequence._createCounter(doc, (createErr, createSeq) => {
        ...
        if (!_.isNull(createSeq)) {
          doc.set(sequence._options.stringSequenceField, generateSequenceString(createSeq, sequence._options))
          ...
          sequence._setNextCounter(doc, (setError, setSeq) => {
            ...
            doc.set(sequence._options.stringSequenceField, generateSequenceString(setSeq, sequence._options))
            doc.set(sequence._options.inc_field, setSeq);
            cb();
          });
        }
      });
      function generateSequenceString(seqNo,options) {
        const paddedSeqNo = _.padStart(seqNo + "", options.length, "0");
        return `${options.prefix}${paddedSeqNo}${options.suffix}`
      }
    };
  };
